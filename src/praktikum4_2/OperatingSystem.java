...
public class OperatingSystem {
    ...
    /**
     * Seitengrï¿½ï¿½e (default: 2^8 Byte = 256 Byte Seitengrï¿½ï¿½e --> max.
     * 2^8 = 256 Seitenrahmen , 64 Worte pro Seitenrahmen)
     */
    ...
    /**
     * Platteneigenschaften: Plattengrï¿½ï¿½e = virt. Adressraum reicht hier,
     * weil wir keine weiteren Dateien brauchen
     */
    ...
    private void loadProcess(int pid, int processSize) {
        ...
        System.out.println(
            "Prozess " + pid + ": " + processSize + " Byte (" + processSize / getPAGE_SIZE()
    ...
    public synchronized int write(int pid, int virtAdr, int item) {
        ...
        if ((virtAdr < 0) || (virtAdr > VIRT_ADR_SPACE - WORD_SIZE)) {
            System.err.println("OS: write ERROR " + pid + ": Adresse " + virtAdr
                + " liegt auï¿½erhalb des virtuellen Adressraums 0 - " + VIRT_ADR_SPACE);
            return -1;
        }
        // Seitenadresse berechnen
        virtualPageNum = getVirtualPageNum(virtAdr);
        offset = getOffset(virtAdr);
        testOut("OS: write " + pid + " " + virtAdr + " " + item + " +++ Seitennr.: "
        ...
        testOut(
            "OS: write " + pid + " +++ item: " + item + " erfolgreich an virt. Adresse " + virtAdr
    ...
    private Process getProcess(int pid) {
        testOut("OS: getProcess " + processTable.get(pid) + " +++ from PID: " + pid);
    ...
    private int getVirtualPageNum(int virtAdr) {
        /*
         * Um die SeitenZahl zu berechnen, muss man die adresse durch die anzahl
         * der seitengröße teilen. Bei einer Seitengröße von 256 gehen die
         * Adressen von Adresse 0 bis 256. 0 ╔ ║ Seite 0 ║ 256 ╚ 257 ╔ ~300 -> ║
         * Seite 1 ║ 512 ╚ Als beispiel: wenn wir die Adresse 300 bekommen, so
         * teilen wir 300 durch 256. 300/256=1.171875 Wenn nur der Wert vor dem
         * Komma betrachtet wird, sagt dies 1. Die Adresse befindet sich also
         * auf der Seite 1.
         */
    ...
    private int getOffset(int virtAdr) {
        /*
         * Um den Offset zu errechnen, Wird eine ganzzahlige Division mit rest
         * durchgeführt. Der Rest ist der Offset. Bei einer Fenstergröße von 256
         * wäre bei der Adresse 300 300%256=44 der Offset 44
         */
    ...
    private PageTableEntry handlePageFault(PageTableEntry pte, int pid) {
        ...
        pte.valid = true;
        testOut(
            "OS: " + pid + " +++ Seite " + pte.virtPageNum + " ist wieder im RAM mit Startadresse "
                + pte.realPageFrameAdr);
    ...
    private int getNewRAMPage(PageTableEntry newPte, int pid) {
        // Algorithmus:
        // Anforderung einer neuen RAM-Seite fï¿½r die gegebene newPte
        // erfï¿½llbar?
        ...
        PageTableEntry replacePte; // Eintrag fï¿½r eine ggf. zu ersetzende
                                   // Seite
        ...
        if (proc.pageTable.getSize() < MAX_RAM_PAGES_PER_PROCESS) {
            ...
            // Nein, Seitenanforderung im RAM ist nicht erfï¿½llbar:
            testOut(
                "OS: getNewRAMPage " + pid + " ++ Seitenfehler fï¿½r Seite " + newPte.virtPageNum
                    + " --> Seitenersetzungs-Algorithmus!");
            // eine alte Seite zur Verdrï¿½ngung auswï¿½hlen -->
            // Seitenersetzungs-Algorithmus
    ...
    private int readFromRAM(int ramAdr) {
        ...
        itemObject = physRAM.get(new Integer(ramAdr));
        if (itemObject == null) {
            result = -1;
        } else {
    ...
    private void dataTransferToDisk(int ramAdr, int diskAdr) {
        ...
        for (ri = ramAdr; ri < ramAdr + PAGE_SIZE; ri = ri + WORD_SIZE) {
            currentWord = physRAM.get(new Integer(ri));
    ...
    private void dataTransferFromDisk(int diskAdr, int ramAdr) {
        ...
        for (di = diskAdr; di < diskAdr + BLOCK_SIZE; di = di + WORD_SIZE) {
            currentWord = physDisk.get(new Integer(di));
    ...
    /**
     * Lï¿½sche einen Plattenblock und trage ihn in die
     * Platten-Freibereichsliste ein
     * 
     * @param diskAdr
     */
    ...
    public synchronized void setMAX_RAM_PAGES_PER_PROCESS(int i) {
        ...
        testOut(
            "OS: MAX_RAM_PAGES_PER_PROCESS: " + MAX_RAM_PAGES_PER_PROCESS + " MAX_NUM_OF_PROCESSES:"
